...
const StyleGallery = ({ items, ariaLabel = 'Gallery', className }: StyleGalleryProps) => {
  ...
  const [activeTheme, setActiveTheme] = useState(ALL_FILTER);
  ...
  const themes = useMemo(() => getUniqueOptions(items, (item) => item.theme), [items]);
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      const matchesTheme = activeTheme === ALL_FILTER || item.theme === activeTheme;

      return matchesStyle && matchesPetType && matchesTheme;
    });
  }, [items, activeStyle, activePetType, activeTheme]);

  const hasActiveFilters =
    activeStyle !== ALL_FILTER || activePetType !== ALL_FILTER || activeTheme !== ALL_FILTER;

  const handleClearFilters = () => {
    setActiveStyle(ALL_FILTER);
    setActivePetType(ALL_FILTER);
    setActiveTheme(ALL_FILTER);
  };
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-body-sm text-muted-foreground">
          Showing {filteredItems.length} of {items.length} portraits
        </p>
        {hasActiveFilters && (
      ...
      {filteredItems.length > 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-border/60 bg-muted/30 p-10 text-center">
          ...
          <p className="text-body-sm text-muted-foreground mt-2">
            Try adjusting your style, pet type, or theme to see more options.
          </p>
